...
def transform2origin_robust(position_tensor, scale=1.0, q_low=0.01, q_high=0.99):
    """
    基于分位数包围盒的鲁棒归一化：
    - 用 [q_low, q_high] 分位代替 min/max，降低离群散点对中心和尺度的影响
    - 中心取分位盒中心，尺度取分位盒最长边
    """
    q_low = float(max(0.0, min(0.49, q_low)))
    q_high = float(max(0.51, min(1.0, q_high)))
    if q_high <= q_low:
        q_low, q_high = 0.01, 0.99

    pos = position_tensor
    low = torch.quantile(pos, q_low, dim=0)
    high = torch.quantile(pos, q_high, dim=0)
    max_diff = torch.max(torch.clamp(high - low, min=1e-12))
    original_mean_pos = (low + high) / 2.0

    scale = torch.as_tensor(scale, dtype=pos.dtype, device=pos.device) / max_diff
    new_position_tensor = (pos - original_mean_pos) * scale
    return new_position_tensor, scale, original_mean_pos
